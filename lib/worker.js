(function() {
  var TaskExtension, func, getRate, module, path, progress, startTime, task, totalFinished, totalStarted, waitingForMore;

  path = require('path');

  module = path.join(process.cwd(), process.argv[2]);
  ...
  func = function(data) {
    if (data === '') {
      ...
      progress.started++;
      totalStarted++;
      return task.emit('worker:message', data);
    }
  };
